...
public class StrikersAdapter extends ArrayAdapter<Striker> implements OnClickListener {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
        ...
        int badgeRes = R.drawable.badge_dummy;
        String clubName = striker.getClubNameIfDummy();
        try{        	
        	badgeRes = striker.getClub().getBadgeResource(getContext());
        	clubName = striker.getClub().getName();
        }catch(NullPointerException npe) { } //o striker nao tem clube, o que significa que o logo deve ser dummy
        
        final String toastClubName = clubName;
        
        holder.tvStrikerName.setText(striker.getName());
        holder.ivClubBadge.setImageBitmap(BitmapOptimisator.decodeSampledBitmapFromResource(getContext().getResources(),
        		badgeRes, 100, 100));
        holder.ivClubBadge.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				Toast.makeText(v.getContext(), toastClubName, Toast.LENGTH_SHORT).show();
			}
		});
