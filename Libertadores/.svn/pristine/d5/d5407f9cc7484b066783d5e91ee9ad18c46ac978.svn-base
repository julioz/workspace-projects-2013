...
public class MatchesOrderedListActivity extends ListActivity implements AsyncTaskListener {
	...
	protected void onListItemClick(ListView l, View v, int position, long id) {
		if(l.getAdapter() instanceof MatchesListAdapter){			
			...
			if(null != detailsUrl){
				if(match.getHome().getCountry() == Country.BRAZIL ||
						match.getAway().getCountry() == Country.BRAZIL){
	...
	public void onComplete(Object result) {
		...
		for (Phase phase : Phase.values()) {
			ArrayList<Match> phaseMatches = matches.get(phase.getNumber());
			if(phaseMatches != null){				
				saveMatches(phase.getNumber(), phaseMatches);
