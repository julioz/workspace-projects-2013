...
public class StrikersREST {
	...
	private final String STRIKERS_DIVISION_A_URL =
			...
			// "http://esporte.uol.com.br/futebol/campeonatos/brasileirao/artilheiros/";
			"http://www.df.superesportes.com.br/campeonatos/2013/brasileiro/serie-a/artilharia_297/";
	private final String STRIKERS_DIVISION_B_URL =
	//		"http://globoesporte.globo.com/futebol/brasileirao-serie-b/#/artilharia";
	"http://esporte.uol.com.br/futebol/campeonatos/brasileiro/2013/serie-b/artilheiros";
	...
	public ArrayList<Striker> getStrikers(Division division){
		if (division == Division.FIRSTDIVISION) {
			return getUolFirstDivision();
		} else {
			return getUolSecondDivision();
		}
	}

	private ArrayList<Striker> getUolFirstDivision() {
		ArrayList<Striker> strikers = new ArrayList<Striker>();
		
		try {
			Document document = Jsoup.connect(STRIKERS_DIVISION_A_URL).get();
			Element tableClassificacao = document.getElementsByClass("table_classificacao").get(0);
			Elements tableRows = tableClassificacao.getElementsByTag("tr");
			tableRows.remove(0); // headers
			
			for (Element tableRow : tableRows) {
				Elements tableDatas = tableRow.getElementsByTag("td");
				Element tdName = tableDatas.get(0);
				Element tdGoals = tableDatas.get(1);
				Element tdClub = tableDatas.get(2);
				
				String name = tdName.text().toString().trim();
				Integer goals = Integer.valueOf(tdGoals.text().toString().trim());
				if (goals <= STRIKER_MINGOALS){
					break;
				}
				Club club = webDatabaseMapper.getClubByName(tdClub.text().toString().trim());
				
				Striker striker = new Striker(name, club, goals);
	...
	public ArrayList<Striker> getUolSecondDivision() {
		ArrayList<Striker> al = new ArrayList<Striker>();
		
		try{
			Document doc = Jsoup.connect(STRIKERS_DIVISION_B_URL).get();
