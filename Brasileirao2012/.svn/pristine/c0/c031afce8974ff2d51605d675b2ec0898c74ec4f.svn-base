...
public class HTMLManager {
	...
	public static JSONObject getTableData(Context c, MatchesDB matchesDB,
		...
		for (int rodada = rodDates.size(); rodada >= 1; rodada--) {
			Calendar date = (Calendar) rodDates.get(rodada).clone();
			date.add(Calendar.DAY_OF_MONTH, -2);
			String dateString = date.get(Calendar.DAY_OF_MONTH)
					+ "/" + Integer.valueOf(date.get(Calendar.MONTH)+1);
			
			// nao preciso tentar atualizar as partidas que ainda nao aconteceram
			// ou dois dias antes
			if(today.before(date)){
				Log.d(Constants.TAG, today.get(Calendar.DAY_OF_MONTH)
						+ "/" + Integer.valueOf(today.get(Calendar.MONTH)+1) + " before rodada [-2] " + rodada
						+ " (" + dateString + ")");
			}else{
				try{
	...
	public static String getNewsContent(String newsurl, NewsDomain domain) {
		...
		try{
			if(domain == NewsDomain.GLOBOESPORTE){
				...
				return ret;
			}else if(domain == NewsDomain.NETFLU){
				return parseNetFlu(newsurl);
	...
	private static String parseNetFlu(String newsurl) throws IOException {
		Document document = Jsoup.connect(newsurl).get();
		Element noticias = document.getElementById("noticias");
		Elements paragraph = noticias.getElementsByTag("p");
		
		try{
			String ret = new String();
			for (int i = 0; i < paragraph.size(); i++) {
				String html = paragraph.get(i).html();
			...
			Log.e(Constants.TAG, "****"+ret.replaceAll("<br />", "").trim().length());
			if(ret.replaceAll("<br />", "").trim().length() <= 330) {
				return null;
			}
			
			return ret;
		} catch(Exception e) {
			return noticias.text();
		}
	}
