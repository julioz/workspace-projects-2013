...
public class MatchesParser {
	...
	private void parseMatches(Node matches, Fixture f) {
		...
		for (Node node : nodeMatches) {
			...
			GregorianCalendar gregCal = new GregorianCalendar(TimeZone.getTimeZone(Constants.TIMEZONE));
			gregCal.set(Integer.valueOf(strYear), Integer.valueOf(strMonth)-1, Integer.valueOf(strDate), 0, 0);
			...
			try{
				...
				Log.e(Constants.TAG, "Home = " + homeAcronym + ", Away = " + awayAcronym);
			}
