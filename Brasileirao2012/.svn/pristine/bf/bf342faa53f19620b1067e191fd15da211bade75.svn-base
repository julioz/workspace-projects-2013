...
public class RankingREST {
	...
	private final String RANKING_DIVISION_A_URL = "http://esporte.uol.com.br/futebol/campeonatos/brasileiro/2013/serie-a/classificacao/fase-unica/";
	private final String RANKING_DIVISION_B_URL = "http://esporte.uol.com.br/futebol/campeonatos/brasileiro/2013/serie-b/classificacao/fase-unica/";
	
	private final String RANKING_REALTIME_DIVISION_A_URL = "http://infobola.com.br/brasileiro/2013/classificacao.aspx";
	private final String RANKING_REALTIME_DIVISION_B_URL = "http://infobola.com.br/brasileiroserieb/2013/classificacao.aspx";
	...
	public ArrayList<Club> getRealTimeRanking(Division division) {
		...
		for (Element element : tr) {
			...
			td.remove(0); // remove o td com a posicao do clube
			club.setName(td.get(0).text());
			club.setAcronym(webDatabaseMapper.getClubByName(club.getName()).getAcronym());
	...
	public ArrayList<Club> getFullRanking(Division division){
		...
		for (int i = 0; i < oldRanking.size(); i++) {
			...
			for (int j = 0; j < newRanking.size(); j++) {
				...
				if(c2 != null && c2.getAcronym().equals(c.getAcronym())){
