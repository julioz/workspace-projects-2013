...
import br.com.zynger.brasileirao2012.model.RealTimeMatch.State;
...
public class RealTimeREST {
	...
	public ArrayList<RealTimeMatch> getRealTimeMatches(Division division,
		...
		try{
			...
			ArrayList<RealTimeMatch> realTimeMatchesArray = new ArrayList<RealTimeMatch>();
			if(rows.size() > 0){
				ArrayList<Club> fullRankingArrayList = null;
				try{
					if(showClubsPositions){						
						fullRankingArrayList = new RankingREST(clubs).getFullRanking(division);
					}
				...
				for (Element el : rows) {
					...
					if(fullRankingArrayList != null){						
						for (Club club : fullRankingArrayList) {
					...
					realTimeMatchesArray.add(m);
				}
			}
			
			return realTimeMatchesArray;
		}catch(IOException e){
			e.printStackTrace();
			return null;
	...
	private RealTimeMatch parseRealTimeMatch(Element el, Division division) {
		...
		Club home = webDatabaseMapper.getClubByName(team1.trim().toLowerCase(Locale.getDefault()));
		Club away = webDatabaseMapper.getClubByName(team2.trim().toLowerCase(Locale.getDefault()));
		
		State state = State.TO_START;
		if(el.className().equals("tr-status-iniciar")) state = State.TO_START;
		else if(el.className().equals("tr-status-andamento")) state = State.IN_PROGRESS;
		else if(el.className().equals("tr-status-encerrado")) state = State.FINISHED;
	...
	public static GregorianCalendar getGregorianCalendar(String datetime){
		try{
			...
			GregorianCalendar gc = new GregorianCalendar(Constants.YEAR, month-1, day, hour, minute);
			gc.setTimeZone(TimeZone.getTimeZone(Constants.TIMEZONE));
