...
public class QuickAction extends CustomPopupWindow {
	...
	private void createActionList() {
		...
		for (int i = 0; i < actionList.size(); i++) {
			...
			subtitle	= actionList.get(i).getSubtitle();
			icon 		= actionList.get(i).getIcon();
			listener	= actionList.get(i).getListener();
	
			if(subtitle != null && icon == null){
				view		= getListViewItem(title, subtitle, listener);
			}else{				
	...
	private View getListViewItem(String title, String subtext, OnClickListener listener){
		TableLayout container = (TableLayout) inflater.inflate(R.layout.quickaction_row_listview, null);
		
		TextView tt			= (TextView) container.findViewById(R.id.fieldName);
		TextView st			= (TextView) container.findViewById(R.id.fieldSub);
		
		if(title != null) tt.setText(title);
		if(subtext != null) st.setText(subtext);
