...
public class ViewFlinger extends ViewGroup {
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        if (mIsVerbose) {
            Log.v(TAG, "onInterceptTouchEvent: " + (ev.getAction() & ViewFlingerMotionEventUtils.ACTION_MASK));
        }
        if (((action & ViewFlingerMotionEventUtils.ACTION_MASK) == MotionEvent.ACTION_MOVE)
        ...
        switch (action & ViewFlingerMotionEventUtils.ACTION_MASK) {
            case MotionEvent.ACTION_MOVE: {
                 ...
                 * of the down event.
                 */
                final int pointerIndex = ViewFlingerMotionEventUtils.findPointerIndex(ev, mActivePointerId);
                final float x = ViewFlingerMotionEventUtils.getX(ev, pointerIndex);
                final float y = ViewFlingerMotionEventUtils.getY(ev, pointerIndex);
            ...
            case MotionEvent.ACTION_DOWN: {
                ...
                mActivePointerId = ViewFlingerMotionEventUtils.getPointerId(ev, 0);
            ...
            case ViewFlingerMotionEventUtils.ACTION_POINTER_UP:
    ...
    void onSecondaryPointerUp(MotionEvent ev) {
        final int pointerIndex = (ev.getAction() & ViewFlingerMotionEventUtils.ACTION_POINTER_INDEX_MASK) >>
                ViewFlingerMotionEventUtils.ACTION_POINTER_INDEX_SHIFT;
        final int pointerId = ViewFlingerMotionEventUtils.getPointerId(ev, pointerIndex);
        if (pointerId == mActivePointerId) {
            // This was our active pointer going up. Choose a new
            // active pointer and adjust accordingly.
            // TODO: Make this decision more intelligent.
            final int newPointerIndex = pointerIndex == 0 ? 1 : 0;
            mDownMotionX = ViewFlingerMotionEventUtils.getX(ev, newPointerIndex);
            mDownMotionX = ViewFlingerMotionEventUtils.getY(ev, newPointerIndex);
            mDownScrollX = getScrollX();
            mActivePointerId = ViewFlingerMotionEventUtils.getPointerId(ev, newPointerIndex);
            if (mVelocityTracker != null) {
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        if (mIsVerbose) {
            Log.v(TAG, "onTouchEvent: " + (ev.getAction() & ViewFlingerMotionEventUtils.ACTION_MASK));
        }
        ...
        switch (action & ViewFlingerMotionEventUtils.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mTouchState == TOUCH_STATE_SCROLLING) {
                    ...
                    final int pointerIndex = ViewFlingerMotionEventUtils
            ...
            case MotionEvent.ACTION_UP:
                if (mTouchState == TOUCH_STATE_SCROLLING) {
                    ...
                    final int pointerIndex = ViewFlingerMotionEventUtils.findPointerIndex(ev, activePointerId);
                    final float x = ViewFlingerMotionEventUtils.getX(ev, pointerIndex);
